...
export async function submitRoleBriefAction(formData: FormData) {
  ...
  redirect(`${returnTo}?sent=1&lead=${encodeURIComponent(lead.id)}&cat=${encodeURIComponent(category)}&feedback=${encodeURIComponent(matchFeedbackToken(lead.id))}${talent}${shortlist}`);
}
