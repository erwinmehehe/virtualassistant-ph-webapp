...
export default async function WorkReadinessPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  const { data: setups, error } = await admin.from("va_profiles")
  ...
  const ids = (setups || []).map((row: any) => row.user_id);
  const { data: profiles } = ids.length
    ? await admin.from("profiles").select("id,full_name,account_status").in("id", ids)
    : { data: [] as any[] };
  const names = new Map((profiles || []).map((row: any) => [row.id, row]));
  const rows = (setups || []).filter((row: any) => names.get(row.user_id)?.account_status === "active");
  const pending = rows.filter((row: any) => !row.work_setup_verified_at);
  const verified = rows.filter((row: any) => row.work_setup_verified_at);
