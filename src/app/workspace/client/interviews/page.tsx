...
export default async function ClientInterviewsPage({searchParams}:{searchParams:Promise<Record<string,string|undefined>>}){
  ...
  return <div className="client-interviews-page">
    ...
    <div className="page-head client-interviews-head"><div><div className="kicker">Hiring interviews</div><h1>Candidate interviews</h1><p>Choose a time in your own timezone, join Google Meet, then record a simple Proceed, Hold, or Pass decision.</p></div><Link className="btn client-interviews-back" href="/workspace/client/candidates">Back to candidates</Link></div>
    <div className="stack client-interviews-list">
      {active.length?active.map((row)=>{const job:Partial<JobSummaryRow>=jobMap.get(row.job_id)||{};const candidate=String(nameMap.get(row.va_id)||"VA candidate");const canFeedback=Boolean(row.scheduled_at&&new Date(row.scheduled_at).getTime()<=Date.now());return <section className="card client-interview-card" key={row.id}>
        <div className="row-between wrap client-interview-summary"><div><div className="row wrap"><span className="badge">{row.status}</span>{row.client_decision?<span className="badge badge-success">{row.client_decision}</span>:null}</div><h2 style={{margin:"8px 0 4px"}}>{candidate}</h2><p className="small muted">{job.title||"Client role"}{job.company_name?` · ${job.company_name}`:""}</p></div>{row.meeting_url&&row.status==="scheduled"?<a className="btn btn-primary client-interview-join" href={row.meeting_url} target="_blank" rel="noreferrer"><Video size={15}/> Join Google Meet <ExternalLink size={13}/></a>:null}</div>
        {row.status==="requested"?<div className="client-interview-scheduler" style={{marginTop:16}}><CandidateInterviewScheduler interviewId={row.id}/></div>:null}
        {row.status==="scheduled"?<div className="stack client-interview-scheduled" style={{marginTop:16}}><div className="review-answer client-interview-time"><div className="row"><CalendarClock size={16}/><strong>{localLabel(row.scheduled_at,row.timezone)}</strong></div><p className="small muted">Timezone used when scheduled: {row.timezone||job.timezone||"Client local time"} · {row.duration_minutes||30} minutes</p></div><details className="client-interview-reschedule"><summary className="btn btn-sm">Reschedule</summary><div className="client-interview-scheduler" style={{marginTop:12}}><CandidateInterviewScheduler interviewId={row.id} currentIso={row.scheduled_at}/></div></details><form action={cancelCandidateInterviewAction} className="client-interview-cancel"><input type="hidden" name="interview_id" value={row.id}/><input type="hidden" name="return_to" value="/workspace/client/interviews"/><button className="btn btn-sm" type="submit">Cancel interview</button></form></div>:null}
        {canFeedback&&!row.client_decision?<form action={submitCandidateInterviewFeedbackAction} className="stack client-interview-feedback" style={{marginTop:18}}><input type="hidden" name="interview_id" value={row.id}/><div><h3 style={{marginBottom:4}}>Interview decision</h3><p className="small muted">Keep this lightweight so the recruiting team can act immediately.</p></div><div className="field"><label>Decision</label><select name="decision" required defaultValue="hold"><option value="proceed">Proceed</option><option value="hold">Hold</option><option value="pass">Pass</option></select></div><div className="field"><label>Reason / theme</label><select name="feedback_reason" defaultValue=""><option value="">Choose if useful</option><option value="skills">Skills</option><option value="rate">Rate</option><option value="schedule / timezone">Schedule / timezone</option><option value="experience">Experience</option><option value="communication">Communication</option><option value="industry fit">Industry fit</option><option value="availability">Availability</option><option value="client preference">Client preference</option><option value="other">Other</option></select></div><div className="field"><label>Feedback for recruiter</label><textarea name="feedback" maxLength={3000} placeholder="What worked, what did not, or what you need clarified before deciding."/></div><button className="btn btn-primary" type="submit">Save interview decision</button></form>:null}
        {row.client_decision?<div className="review-answer client-interview-decision" style={{marginTop:16}}><strong>Client decision: {row.client_decision}</strong>{row.client_feedback_reason?<p>Reason: {row.client_feedback_reason}</p>:null}{row.client_feedback?<p className="small muted">{row.client_feedback}</p>:null}</div>:null}
