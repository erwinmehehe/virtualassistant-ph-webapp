...
export default async function ClientMessagesPage({searchParams}:{searchParams:Promise<Record<string,string|undefined>>}){
  ...
  return <>
    ...
    {!rows.length ? <section className="workspace-empty-card" aria-labelledby="client-messages-empty-title">
      ...
      <div className="row wrap workspace-empty-actions"><a className="btn btn-primary" href={HIRING_CALL_URL} target="_blank" rel="noopener noreferrer">Book a 15-minute hiring call</a><Link className="btn" href="/workspace/client/candidates"><UsersRound size={16}/>Review candidates</Link><Link className="btn" href="/workspace/client/jobs"><BriefcaseBusiness size={16}/>View roles</Link></div>
