...
export default async function ManageBookingPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  return <><SiteHeader/><main id="main-content" className="booking-page"><section className="booking-hero"><div className="container booking-container"><span className="kicker">Client discovery call</span><h1>{lead.discovery_outcome === "no_show" ? "Choose another time" : "Manage your booking"}</h1><p>{lead.company}{lead.discovery_scheduled_at ? ` · ${formatDiscoverySlot(lead.discovery_scheduled_at, lead.timezone || "Asia/Manila")}` : ""}</p></div></section><section className="section section-white booking-main-section"><div className="container booking-container">{query.cancelled ? <div className="alert success">Your booking is cancelled.</div> : null}{query.rescheduled ? <div className="alert success">Your booking was rescheduled.</div> : null}{query.error ? <div className="alert error">We could not update that booking. Please choose another time or contact the hiring team.</div> : null}{lead.discovery_cancelled_at ? <p>This call is cancelled.</p> : <ManageBookingForm token={token} days={days} rebookOnly={lead.discovery_outcome === "no_show"}/>}</div></section></main><SiteFooter/></>;
}
