...
export default async function FindTalentPage({ searchParams }: { searchParams: Promise<Record<string,string|undefined>> }) {
  ...
  return <><SiteHeader/><main id="main-content" className="talent-directory-page">
    ...
    <section className="section directory-section"><div className="container">
      ...
      <div className="directory-result-head"><div><strong>{vas.length} available profile{vas.length === 1 ? "" : "s"}</strong><span className="small muted">Only approved, available VAs with 2+ years of experience are shown.</span></div></div>
      {vas.length ? <div className="talent-directory-grid">{vas.map((va:any)=><article className="talent-market-card" key={va.user_id}>
      ...
      </article>)}</div> : <div className="card empty"><h3>No profiles match those filters.</h3><p>Try a broader specialty or availability range, or send us the role and we can match the approved pool directly.</p><div className="row wrap" style={{justifyContent:"center"}}><Link className="btn" href="/find-talent">Clear filters</Link><Link className="btn btn-primary" href="/hire">Get a managed VA</Link></div></div>}
    </div></section>
  </main><SiteFooter/></>;
}
