...
type AdminJobRow = { id: string; title: string | null; company_name: string | null; status: string; service_model: string | null; client_id: string | null; recruiter_id: string | null; hours_per_week: number | null; min_hourly_rate: number | null; created_at: string };
...
export default async function AdminJobsPage({searchParams}:{searchParams:Promise<Record<string,string|undefined>>}){
  ...
  return <>
    ...
    <div className="stack">{visible.length?visible.map((job)=>{const commercial=cm.get(job.id);const owner=job.recruiter_id?recruiterMap.get(job.recruiter_id):null;return <article className="card" key={job.id}>
