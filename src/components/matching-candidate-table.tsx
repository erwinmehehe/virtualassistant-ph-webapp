  ...
  return <>
    ...
    <div className="table-wrap responsive-table matching-table" style={{maxHeight:"none",overflowX:"auto",overflowY:"visible"}}><table>
      ...
      <tbody>{visible.map((row) => {
        ...
        return <tr key={row.va.user_id} className={alreadyReleased ? "released-match-row" : undefined}>
          <td data-label="Select"><label className="compare-check"><input type="checkbox" name="va_id" value={row.va.user_id} defaultChecked={selected} disabled={hardBlocked&&!alreadyReleased}/><span className="sr-only">Select {row.account?.full_name || "VA"}</span></label></td>
          ...
          <td data-label="VA"><strong>{row.account?.full_name || "VA candidate"}</strong><div className="small muted">{row.va.headline || row.va.primary_category || "Virtual Assistant"}</div><div className="pill-list compact-pills">{mergeUniqueStrings(row.va.primary_category, row.va.categories).slice(0, 2).map((x: string, i: number) => <span className="badge" key={`${x}-${i}`}>{x}</span>)}</div>
            ...
            <div style={{marginTop:8}}><Link className="text-link small" href={`/workspace/recruiter/candidates/${row.va.user_id}/screening`}>Open recruiter scorecard</Link></div>
          </td>
          ...
          <td data-label="Client recommendation"><textarea name={`recommendation_${row.va.user_id}`} defaultValue={row.shortlist?.client_recommendation || ""} maxLength={500} rows={3} placeholder="Why this VA is a strong fit for this client..." disabled={hardBlocked&&!alreadyReleased}/>{!hardBlocked||alreadyReleased?<button className="text-button" type="submit" formAction={saveClientRecommendationAction} name="recommendation_va_id" value={row.va.user_id}>Save client note</button>:<span className="small muted">Change the hard requirement before recommending.</span>}</td>
