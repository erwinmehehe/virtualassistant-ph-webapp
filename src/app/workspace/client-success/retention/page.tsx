...
type JobRow = { id: string; title: string | null; company_name: string | null; recruiter_id: string | null };
...
export default async function RetentionAndReplacementPage({ searchParams }: { searchParams: Promise<Record<string,string|undefined>> }) {
  ...
  const [{ data: roomData, error: roomError }, { data: replacementData, error: replacementError }] = await Promise.all([
    admin.from("workrooms").select("id,job_id,client_id,va_id,client_success_owner_id,placement_stage,health_status,health_score,renewal_date,renewal_status,end_reason,offboarding_notes,created_at").order("created_at", { ascending: false }).limit(200),
    admin.from("placement_support_requests").select("id,workroom_id,status,details,resolution,replacement_reason,replacement_sla_due_on,guarantee_status,guarantee_notes,created_at").eq("request_type", "replacement").order("created_at", { ascending: false }).limit(200),
  ]);
  if (roomError) throw roomError;
  if (replacementError) throw replacementError;

  const rooms = (roomData || []) as RoomRow[];
  const jobIds = [...new Set(rooms.map((room) => room.job_id))];
  const personIds = [...new Set(rooms.flatMap((room) => [room.client_id, room.va_id]).filter((id): id is string => Boolean(id)))];
  const [{ data: jobData }, { data: personData }] = await Promise.all([
    jobIds.length ? admin.from("jobs").select("id,title,company_name,recruiter_id").in("id", jobIds) : Promise.resolve({ data: [] }),
    personIds.length ? admin.from("profiles").select("id,full_name").in("id", personIds) : Promise.resolve({ data: [] }),
  ]);
  const jobs = (jobData || []) as JobRow[];
  const people = (personData || []) as PersonRow[];
  const jobMap = new Map(jobs.map((job) => [job.id, job]));
  const peopleMap = new Map(people.map((person) => [person.id, person]));
  const visibleRooms = rooms.filter((room) => {
    const job = jobMap.get(room.job_id);
    return profile.role === "admin" || room.client_success_owner_id === user.id || job?.recruiter_id === user.id;
  });
  const visibleRoomIds = new Set(visibleRooms.map((room) => room.id));
  const replacements = ((replacementData || []) as ReplacementRow[]).filter((request) => visibleRoomIds.has(request.workroom_id));
  const roomMap = new Map(visibleRooms.map((room) => [room.id, room]));
  ...
  return <>
    ...
    <section className="card" style={{marginTop:18}}>
      ...
      {replacements.length ? <div className="stack" style={{marginTop:14}}>{replacements.map((request) => {
        const room = roomMap.get(request.workroom_id); if (!room) return null;
        const job = jobMap.get(room.job_id); const client = room.client_id ? peopleMap.get(room.client_id)?.full_name : null; const va = room.va_id ? peopleMap.get(room.va_id)?.full_name : null;
        ...
        return <article className="card" key={request.id}>
          <div className="row-between wrap"><div><div className="row wrap"><span className={`badge ${overdue ? "badge-danger" : active ? "badge-warning" : "badge-success"}`}>{request.status}</span>{overdue ? <span className="badge badge-danger">SLA overdue</span> : null}{request.replacement_sla_due_on ? <span className="badge"><CalendarClock size={12}/> Due {request.replacement_sla_due_on}</span> : <span className="badge badge-warning">SLA not set</span>}</div><h3 style={{margin:"8px 0 3px"}}>{job?.title || "Placement"}</h3><p className="small muted" style={{margin:0}}>{job?.company_name || client || "Client"} · {va || "Virtual Assistant"}</p></div><Link className="btn btn-sm" href={`/workspace/client-success/${room.id}`}>Open placement</Link></div>
          <p>{request.details}</p>
          {active ? <>
            <form action={saveReplacementWorkflowAction} className="stack" style={{marginTop:12}}>
              <input type="hidden" name="request_id" value={request.id}/><input type="hidden" name="workroom_id" value={room.id}/><input type="hidden" name="return_to" value="/workspace/client-success/retention"/>
            ...
            <form action={resolvePlacementSupportRequestAction} className="stack" style={{marginTop:12}}><input type="hidden" name="request_id" value={request.id}/><input type="hidden" name="workroom_id" value={room.id}/><div className="form-grid"><div className="field"><label>Case status</label><select name="status" defaultValue={request.status === "open" ? "acknowledged" : request.status}><option value="acknowledged">Acknowledge</option><option value="resolved">Resolve</option><option value="declined">Decline</option></select></div><div className="field span-2"><label>Outcome / next step</label><textarea name="resolution" maxLength={4000} defaultValue={request.resolution || ""} placeholder="Who owns the replacement search, what happens to the current placement, and what was agreed with the client?"/></div></div><button className="btn" type="submit">Save case status</button></form>
    ...
    <section className="card" style={{marginTop:18}}>
      ...
      {visibleRooms.length ? <div className="stack" style={{marginTop:14}}>{visibleRooms.slice(0,80).map((room) => {const job=jobMap.get(room.job_id);const client=room.client_id?peopleMap.get(room.client_id)?.full_name:null;const va=room.va_id?peopleMap.get(room.va_id)?.full_name:null;return <article className="card" key={room.id}>
        <div className="row-between wrap"><div><div className="row wrap"><span className="badge">{label(room.placement_stage)}</span><span className="badge">{label(room.renewal_status || "not_set")}</span>{room.renewal_date?<span className="badge"><CalendarClock size={12}/> {room.renewal_date}</span>:null}</div><h3 style={{margin:"8px 0 3px"}}>{job?.title||"Placement"}</h3><p className="small muted" style={{margin:0}}>{job?.company_name||client||"Client"} · {va||"Virtual Assistant"}</p></div><Link className="btn btn-sm" href={`/workspace/client-success/${room.id}`}>Open placement</Link></div>
