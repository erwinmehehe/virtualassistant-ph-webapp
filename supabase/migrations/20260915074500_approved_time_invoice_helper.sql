alter table public.time_entries add column if not exists payment_id uuid references public.payments(id) on delete set null;
create index if not exists time_entries_unbilled_approved_idx on public.time_entries(workroom_id,work_date) where status='approved' and payment_id is null;
...
begin
  ...
  select coalesce(sum(hours),0) into v_hours from time_entries where workroom_id=p_workroom_id and status='approved' and payment_id is null for update;
