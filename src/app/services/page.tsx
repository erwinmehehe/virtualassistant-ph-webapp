...
export default function ServicesPage(){return <><SiteHeader/><main id="main-content">
  <section className="section public-hero-small"><div className="container"><div className="public-page-head"><h1 className="public-page-title">Find the Virtual Assistant role that matches the work you need done.</h1><p className="public-lede">Start with the workload, not a generic job title. Browse role-specific guides for responsibilities, tools, interview questions, and hiring criteria.</p><div className="row wrap"><Link className="btn btn-primary btn-lg" href="/hire">Hire a Virtual Assistant <ArrowRight size={16}/></Link><Link className="btn btn-lg" href="/find-talent">Browse Virtual Assistants</Link></div></div></div></section>

  <section className="service-directory-intro"><div className="container"><nav className="service-category-nav" aria-label="Service categories">{SERVICE_CATEGORIES.map((category)=><a href={`#${category.id}`} key={category.id}>{category.label}</a>)}</nav><div className="service-role-help"><div><Search size={20}/><span><strong>Not sure which role fits?</strong><small>Describe what is falling behind and we will help you narrow the role.</small></span></div><Link className="btn btn-primary" href="/hire">Hire a Virtual Assistant <ArrowRight size={15}/></Link></div></div></section>
  ...
  <section className="section section-white services-bottom-cta"><div className="container row-between wrap"><div><h2>Still comparing roles?</h2><p className="muted">Send the workload instead of guessing the title. Your request stays private until you review it.</p></div><div className="row wrap"><Link className="btn btn-primary" href="/hire">Hire a Virtual Assistant</Link><Link className="btn" href="/pricing">See pricing</Link></div></div></section>
</main><SiteFooter/></>}
