...
export default async function RecruiterDashboard(){
  ...
  const [unreviewedRes,incompleteRes,readyRes,vettedHiddenRes,activeJobsRes,newAppsRes,unreadMessagesRes,recentLeadsRes,jobsRes,releasedRes]=await Promise.all([
    ...
    admin.from("lead_intake").select("id,created_at",{count:"exact"}).eq("status","new").gte("created_at",since).order("created_at",{ascending:false}).limit(200),
  ...
  const recentLeadIds=(recentLeadsRes.data||[]).map((lead:any)=>lead.id);
  const {data:leadContactActivity}=recentLeadIds.length
    ? await admin.from("recruiter_activity")
        .select("subject_id")
        .eq("subject_type","lead")
        .in("subject_id",recentLeadIds)
        .or("action.like.client_contact_%,action.eq.client_followup_sent")
        .limit(1000)
    : {data:[]} as any;
  const contactedLeadIds=new Set((leadContactActivity||[]).map((row:any)=>row.subject_id));
  const untouchedLeads=(recentLeadsRes.data||[]).filter((lead:any)=>!contactedLeadIds.has(lead.id));
  const untouchedLeadCount=untouchedLeads.length;

  const cards=[
    ["Client leads needing first contact",untouchedLeadCount,"/workspace/recruiter/leads",Mail,"Revenue first: reply before the prospect keeps shopping"],
    ...
    ["Open client leads",recentLeadsRes.count||0,"/workspace/recruiter/leads",Mail,"New enquiries from the last 7 days"]
  ] as const;
  const today=[
    {priority:"urgent",title:"Client leads need first contact",count:untouchedLeadCount,copy:"Reply first. Fast response is the highest-value action in this queue.",href:"/workspace/recruiter/leads"},
  ...
  return <>
    <div className="page-head"><div><div className="kicker">Recruiter control center</div><h1>Today’s work</h1><p>Revenue first: respond to new clients, unblock active roles, then work the talent pipeline.</p></div><div className="row wrap"><Link className="btn" href="/workspace/recruiter/leads">Open client leads</Link><Link className="btn btn-primary" href="/workspace/recruiter/matching">Match active roles</Link></div></div>
