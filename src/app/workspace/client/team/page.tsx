...
export default async function ClientTeamPage({searchParams}:{searchParams:Promise<Record<string,string|undefined>>}){
  ...
  return <div className="client-team-page">
    ...
    <div className="page-head client-team-head"><div><h1>My Team</h1><p>Your active Virtual Assistants, placement health, launch progress, and Client Success support in one place.</p></div></div>
    {active.length?<div className="stack client-team-list">{active.map((room)=>{const job:Partial<JobSummaryRow>=jobMap.get(room.job_id)||{};const va:Partial<AvatarProfileRow>=vaMap.get(room.va_id||"")||{};const setup:{work_setup_verified_at?:string|null}=setupMap.get(room.va_id||"")||{};const csm:Partial<AvatarProfileRow>=csmMap.get(room.client_success_owner_id||"")||{};const roomCheckins=checkins.filter((c)=>c.workroom_id===room.id);const pending=roomCheckins.filter((c)=>!c.client_signal&&c.status!=="skipped");const requested=roomCheckins.find((c)=>c.id===requestedCheckin&&!c.client_signal);const next=requested||pending[0];return <section className="card stack client-team-card" key={room.id}>
      <div className="row-between wrap client-team-summary"><div className="row client-team-person" style={{alignItems:"center"}}><PublicAvatar name={va.full_name||"Virtual Assistant"} src={va.avatar_url} size="sm"/><div><div className="row wrap"><h2 style={{margin:"0 0 3px"}}>{va.full_name||"Virtual Assistant"}</h2>{setup.work_setup_verified_at?<span className="badge badge-success"><BadgeCheck size={13}/>Work setup verified</span>:null}</div><p className="small muted" style={{margin:0}}>{job.title||"Placement"}</p></div></div><div className="row wrap"><span className={`badge ${healthBadge(room.health_status||"")}`}><HeartPulse size={13}/>{HEALTH_LABELS[room.health_status||""]||room.health_status}{room.health_score!=null?` · ${room.health_score}`:""}</span><span className="badge">{STAGE_LABELS[room.placement_stage||""]||room.placement_stage}</span></div></div>

      <div className="grid-4 client-team-facts">
      ...
      {room.health_score==null?<div className="info-banner client-team-health-note"><ShieldCheck size={16}/><div><strong>Health is still building</strong><p style={{margin:"4px 0 0"}}>We wait for enough real check-ins and operating data before showing a score.</p></div></div>:null}

      {next?<form action={submitPlacementPulseAction} className="card stack client-team-checkin"><input type="hidden" name="checkin_id" value={next.id}/><div className="row-between wrap"><div><strong>{pulseLabel(next.checkpoint)} check-in</strong><p className="small muted" style={{margin:"4px 0 0"}}>How is {va.full_name||"your VA"} doing?</p></div><span className="small muted"><CalendarClock size={13}/> {dateLabel(next.due_at)}</span></div><div className="row wrap client-team-checkin-actions"><button className="btn btn-sm btn-primary" type="submit" name="signal" value="green">Great</button><button className="btn btn-sm" type="submit" name="signal" value="yellow">Some concerns</button><button className="btn btn-sm btn-danger" type="submit" name="signal" value="red">Need help</button></div><div className="field"><label>Optional note</label><textarea name="note" maxLength={2000} placeholder="Add context only if it helps Client Success understand what is going well or what needs attention."/></div></form>:<div className="info-banner"><CheckCircle2 size={16}/><span>No client check-in is waiting right now. We will ask again at the next placement milestone.</span></div>}

      <div className="row wrap client-team-actions"><Link className="btn btn-primary" href="/workspace/client/workroom"><MessagesSquare size={15}/> Open placement workroom</Link><Link className="btn" href="/workspace/client/support"><LifeBuoy size={15}/> Placement support</Link></div>
