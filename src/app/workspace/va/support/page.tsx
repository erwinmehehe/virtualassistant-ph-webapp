...
export default async function VaSupportPage({ searchParams }: { searchParams: Promise<Record<string,string|undefined>> }) {
  ...
  return <>
    ...
    {rooms.length?<div className="stack" style={{marginTop:18}}>{rooms.map((room)=>{const job:Partial<JobSummaryRow>=jobMap.get(room.job_id)||{};const recent=requests.filter((r)=>r.workroom_id===room.id);return <section className="card stack" key={room.id}>
