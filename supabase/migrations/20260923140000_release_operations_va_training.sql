-- Release the reviewed Operations Virtual Assistant course.
...
where module_id in (
  ...
  where course_id = (select id from public.training_courses where slug = 'operations-virtual-assistant')
);
...
set
  ...
  content_version = greatest(content_version, 2),
  updated_at = now()
where slug = 'operations-virtual-assistant';
