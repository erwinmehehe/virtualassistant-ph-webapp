...
export default async function ClientCandidatesPage(){
  ...
  const [{data:accessRows},{data:applicationRows},{data:released,error:shortlistError}]=await Promise.all([
    admin.from("job_candidate_access").select("job_id,access_status,access_fee").in("job_id",jobIds),
    admin.from("applications").select("id,job_id,va_id,status,match_score,applied_at").in("job_id",jobIds).order("applied_at",{ascending:false}),
  ...
  const activeJobIds=new Set((jobs||[]).filter((job:any)=>["pending","published"].includes(job.status)).map((job:any)=>job.id));
  const curated=pendingReleasedMatches(released||[],applications).filter(row=>activeJobIds.has(row.job_id));
  ...
  const next=curated.length?{title:`${curated.length} curated match${curated.length===1?"":"es"} ready`,copy:"Your recruiting team has prepared a shortlist. Open your role to review it.",label:"Review shortlist"}:offers?{title:`${offers} decision${offers===1?"":"s"} ready`,copy:"A candidate is waiting for your final hiring decision.",label:"Review decisions"}:interviews?{title:`${interviews} interview${interviews===1?"":"s"} in progress`,copy:"Review candidates and keep the interview process moving.",label:"Review interviews"}:readyToReview?{title:`${readyToReview} candidate${readyToReview===1?"":"s"} ready for review`,copy:"Compare role fit and decide who should move forward.",label:"Review candidates"}:{title:"We’re looking for candidates",copy:"Your recruiting team is matching approved VAs to your role.",label:"View roles"};
  ...
  return <>
    ...
    <section className="candidate-next-action"><div className="candidate-next-icon"><Sparkles size={21}/></div><div><span className="small">Your next action</span><h2>{next.title}</h2><p>{next.copy}</p><small className="muted">{curated.length||offers||interviews||readyToReview?"Waiting on you":"Waiting on our recruiting team"}</small></div><a className="btn btn-primary" href={curated.length?"#curated-matches":offers||interviews||readyToReview?"#candidate-list":"/workspace/client/jobs"}>{next.label}<ArrowRight size={16}/></a></section>
    {shortlistError?<div className="alert" role="alert">Your curated shortlists could not be loaded. Refresh this page before making a hiring decision.</div>:null}
    <ReleasedShortlists jobs={jobs||[]} matches={curated}/>
    <details className="card dashboard-section-card"><summary>Prefer to explore? Browse approved VAs</summary>
    <section id="browse-vas">
