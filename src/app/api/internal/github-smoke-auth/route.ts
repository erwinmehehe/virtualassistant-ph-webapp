      ...
      const { error: vaError } = await admin.from("va_profiles").upsert(
        {
          ...
          bio: "Dedicated non-public automated QA profile used only to verify recruiter and client workflows safely in production. This is not a real candidate.",
          primary_category: "Administrative Support",
          categories: ["Administrative Support"],
          skills: ["Data Entry", "Email Management", "Calendar Management", "Research", "CRM Administration"],
          tools: ["Google Workspace", "Slack", "Microsoft 365"],
