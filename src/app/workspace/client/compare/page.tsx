...
export default async function CompareCandidatesPage({searchParams}:{searchParams:Promise<Record<string,string|string[]|undefined>>}){
  ...
  return <div className="client-compare-page">
    <div className="page-head client-compare-head"><div><div className="row wrap"><Link className="text-link small" href="/workspace/client/candidates">← Recruiter shortlist</Link></div><h1>Compare shortlisted candidates</h1><p>Compare evidence only for candidates your recruiter has intentionally released for client review.</p></div></div>
    {excludedCount?<div className="alert client-compare-alert" style={{marginBottom:16}}>{excludedCount} selected candidate{excludedCount===1?" was":"s were"} excluded because they were not released by your recruiter or the role is not ready for client review.</div>:null}
    {candidates.length<2?<div className="card empty client-compare-empty"><p>Select at least two recruiter-released candidates.</p><Link className="btn btn-primary" href="/workspace/client/candidates">Return to shortlist</Link></div>:<>
      {jobTitles.size>1?<div className="alert client-compare-alert" style={{marginBottom:16}}>You selected candidates from different roles. Each fit label is based on that candidate&apos;s own hiring request.</div>:null}
      <div className="compare-grid client-compare-grid">{candidates.map((application)=>{const profile:CompareProfileSnapshot=application.profile_snapshot||{};const score=Number(application.match_score||0);return <article className="card stack client-compare-card" key={application.id}>
        ...
        <Link className="btn btn-primary client-compare-review" href={`/workspace/client/candidates/${application.id}`}>Review evidence</Link>
      </article>})}</div>
    </>}
  </div>;
}
