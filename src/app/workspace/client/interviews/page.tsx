...
export default async function ClientInterviewsPage({searchParams}:{searchParams:Promise<Record<string,string|undefined>>}){
  ...
  const {data:rows,error}=await admin.from("candidate_interviews").select("*").eq("client_id",user.id).order("created_at",{ascending:false}).limit(100);
  if(error)throw error;
  const jobIds=[...new Set((rows||[]).map((row:any)=>row.job_id))];const vaIds=[...new Set((rows||[]).map((row:any)=>row.va_id))];
  ...
  return <>
    {query.scheduled?<div className="success-banner">Interview scheduled. The VA and client received confirmation.</div>:null}{query.cancelled?<div className="success-banner">Interview cancelled.</div>:null}{query.feedback_saved?<div className="success-banner">Interview feedback saved and the recruiting team was notified.</div>:null}
    <div className="page-head"><div><div className="kicker">Hiring interviews</div><h1>Candidate interviews</h1><p>Choose a time in your own timezone, join Zoom, then record a simple Proceed, Hold, or Pass decision.</p></div><Link className="btn" href="/workspace/client/candidates">Back to candidates</Link></div>
    <div className="stack">
      {active.length?active.map((row:any)=>{const job:any=jobMap.get(row.job_id)||{};const candidate=String(nameMap.get(row.va_id)||"VA candidate");const canFeedback=Boolean(row.scheduled_at&&new Date(row.scheduled_at).getTime()<=Date.now());return <section className="card" key={row.id}>
        <div className="row-between wrap"><div><div className="row wrap"><span className="badge">{row.status}</span>{row.client_decision?<span className="badge badge-success">{row.client_decision}</span>:null}</div><h2 style={{margin:"8px 0 4px"}}>{candidate}</h2><p className="small muted">{job.title||"Client role"}{job.company_name?` · ${job.company_name}`:""}</p></div>{row.meeting_url&&row.status==="scheduled"?<a className="btn btn-primary" href={row.meeting_url} target="_blank" rel="noreferrer"><Video size={15}/> Join Zoom <ExternalLink size={13}/></a>:null}</div>
