...
no_shows as materialized (
  select l.id,l.name,l.email,l.discovery_outcome,l.discovery_rescheduled_at,l.discovery_scheduled_at,
    exists(
...
select jsonb_build_object(
  ...
  'no_shows', coalesce((
    select jsonb_agg(to_jsonb(n) order by n.discovery_scheduled_at desc nulls last)
    from (
      select *
      from no_shows
