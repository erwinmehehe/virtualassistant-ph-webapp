...
function softwareLongFormCopy(page: (typeof softwarePages)[number]) {
  ...
  return {
    ...
    weekThree: `By the third week, the role should begin reducing manager follow-up rather than creating more of it. Expand ownership only after the Virtual Assistant has shown reliable execution on the first queue. Add adjacent work, allow more independent prioritization within defined limits, and introduce a simple daily or weekly summary. A useful summary does not need to be long: work completed, items waiting on someone else, exceptions that need a decision, and anything likely to affect a customer, technician, broker, property manager, finance team, or other downstream owner.`,
    weekFour: `At the end of the first month, review the role as an operating system rather than a list of tasks. Which ${page.software} activities are now consistently owned? Which still depend on a manager? Where are delays coming from? Are permissions appropriate? Are records easier to trust? This is the right time to adjust the job description, add or remove responsibilities, refine response-time expectations, and decide whether the role should remain part-time or expand. The best outcome is not “the Virtual Assistant is busy”; it is that the workflow is more visible, more current, and easier for the local team to manage.`,
    quality: `Measure quality with evidence that comes from the workflow itself. Useful indicators can include turnaround time, percentage of records completed without correction, number of overdue follow-ups, missing-document rate, reopened tasks, billing-ready jobs, unresolved exceptions, or the age of items sitting in a queue. Pick only a few measures that reflect the result you care about. Avoid measuring activity for its own sake. A Virtual Assistant who makes fewer updates but keeps the right records accurate and closes the right administrative loops can be more valuable than someone who simply generates a high volume of clicks.`,
    security: `Treat ${page.software} access the same way you would treat access for any remote team member. Create an individual account where the platform supports it, use the minimum permissions needed for the assigned work, enable multi-factor authentication, and avoid sharing owner or administrator credentials. Document which customer, financial, health, legal, employment, or technical information the Virtual Assistant may view or change. Review access when responsibilities change and remove it promptly when the engagement ends. Your organization remains responsible for its own privacy, security, licensing, and compliance obligations.`,
    communication: `Remote administration works better when communication rules are explicit. Define the channel for routine questions, the channel for urgent exceptions, the expected response window, and the information that must be included in an escalation. For example, instead of “this job has a problem,” require the Virtual Assistant to identify the record, current status, missing input, what has already been checked, and the decision needed from the manager. This makes ${page.software} support easier to supervise and reduces the back-and-forth that often makes outsourced administration feel slower than keeping the work in-house.`,
    capacity: `The right weekly hours depend on transaction volume, seasonality, live coverage requirements, and how many adjacent tasks sit around ${page.software}. A small team may start with a focused part-time queue, while a busier operation may need a full-time Virtual Assistant who also owns inbox follow-up, document preparation, CRM hygiene, scheduling, reporting, or customer updates. Estimate capacity from real work: count the number of recurring records or cases, sample the average handling time, add time for follow-up and exceptions, then leave room for training and quality checks.`,
    fit: `This role is a strong fit when your team already has a repeatable process but managers or specialists are still spending too much time maintaining the system. It is also useful when records fall behind during busy periods, follow-up depends on individual memory, or field and office teams do not have a consistent administrative handoff. The role is a weaker fit when the process itself is still undefined, when nearly every item requires senior judgment, or when the Virtual Assistant would be expected to make regulated, technical, financial, or commercial decisions that belong with an appropriately authorised person.`,
