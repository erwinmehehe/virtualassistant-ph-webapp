...
export default async function VaInterviewsPage({searchParams}:{searchParams:Promise<Record<string,string|undefined>>}){
  ...
  return <div className="va-interviews-page">
    {query.cancelled?<div className="success-banner">Interview cancelled. The client has been notified.</div>:null}
    <div className="page-head va-interviews-head"><div><div className="kicker">Client opportunities</div><h1>Interviews</h1><p>See upcoming client interviews, Google Meet links, and scheduling details.</p></div><Link className="btn va-interviews-applications" href="/workspace/va/applications">Applications</Link></div>
    <div className="stack">{active.length?active.map((row)=>{const job:Partial<JobSummaryRow>=jobMap.get(row.job_id)||{};return <section className="card va-interview-card" key={row.id}><div className="row-between wrap va-interview-summary"><div><div className="row wrap"><span className="badge">{row.status}</span>{row.client_decision?<span className="badge">Client: {row.client_decision}</span>:null}</div><h2 style={{margin:"8px 0 4px"}}>{job.title||"Client role"}</h2><p className="small muted">{job.company_name||"Client"}</p></div>{row.meeting_url&&row.status==="scheduled"?<a className="btn btn-primary va-interview-join" href={row.meeting_url} target="_blank" rel="noreferrer"><Video size={15}/> Join Google Meet <ExternalLink size={13}/></a>:null}</div>{row.status==="requested"?<div className="alert" style={{marginTop:14}}>The client requested an interview and is choosing a time. You will be notified when it is scheduled.</div>:null}{row.status==="scheduled"?<div className="review-answer va-interview-schedule" style={{marginTop:14}}><div className="row"><CalendarClock size={16}/><strong>{localLabel(row.scheduled_at)}</strong></div><p className="small muted">Shown in your device timezone. Scheduled from {row.timezone||job.timezone||"client local time"} · {row.duration_minutes||30} minutes.</p><form action={cancelCandidateInterviewAction} style={{marginTop:10}}><input type="hidden" name="interview_id" value={row.id}/><input type="hidden" name="return_to" value="/workspace/va/interviews"/><button className="btn btn-sm" type="submit">I need to cancel</button></form></div>:null}{row.client_feedback?<div className="review-answer" style={{marginTop:14}}><strong>Client feedback recorded</strong><p className="small muted">Your recruiter will handle the next step with the client.</p></div>:null}</section>}):<div className="empty">No client interviews yet.</div>}</div>
  </div>;
}
