...
export default async function ClientJobDetail({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<Record<string,string|undefined>>}){
  ...
  if(releasedIds.length){
    const {data:vaRows}=await admin.from("va_profiles").select("user_id,headline,primary_category,categories,skills,tools,years_experience,weekly_hours,hourly_rate,availability_status,directory_visible,slug").in("user_id",releasedIds);
    shortlistVas=vaRows||[];
    if(unlocked){
      ...
      shortlistProfiles=results[0].data||[]; invites=results[1].data||[];
    }
  }
  ...
  return <>
    ...
    {query.invited?<div className="success-banner" role="status"><CheckCircle2 size={17}/> Invitation sent. The Virtual Assistant can review it from their Applications workspace.</div>:null}
