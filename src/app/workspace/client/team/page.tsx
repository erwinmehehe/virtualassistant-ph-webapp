...
export default async function ClientTeamPage({searchParams}:{searchParams:Promise<Record<string,string|undefined>>}){
  ...
  const [{data:jobs},{data:vas},{data:vaSetups},{data:csms},{data:checkins}]=await Promise.all([
    ...
    vaIds.length?admin.from("va_profiles").select("user_id,work_setup_verified_at").in("user_id",vaIds):Promise.resolve({data:[] as any[]}),
  ...
  const setupMap=new Map((vaSetups||[]).map((x:any)=>[x.user_id,x]));
  ...
  return <>
    ...
    {active.length?<div className="stack">{active.map((room:any)=>{const job:any=jobMap.get(room.job_id)||{};const va:any=vaMap.get(room.va_id)||{};const setup:any=setupMap.get(room.va_id)||{};const csm:any=csmMap.get(room.client_success_owner_id)||{};const roomCheckins=(checkins||[]).filter((c:any)=>c.workroom_id===room.id);const pending=roomCheckins.filter((c:any)=>!c.client_signal&&c.status!=="skipped");const requested=roomCheckins.find((c:any)=>c.id===requestedCheckin&&!c.client_signal);const next=requested||pending[0];return <section className="card stack" key={room.id}>
      ...
      <div className="row wrap"><Link className="btn btn-primary" href="/workspace/client/workroom"><MessagesSquare size={15}/> Open placement workroom</Link><Link className="btn" href="/workspace/client/support"><LifeBuoy size={15}/> Placement support</Link><Link className="btn" href="/workspace/client/messages">Message the team</Link></div>
