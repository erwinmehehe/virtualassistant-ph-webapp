...
export default async function VaNotificationsPage(){
  ...
  return <div className="va-notifications-page">
    <div className="page-head va-notifications-head"><div><h1>Notifications</h1><p>Recruiter requests, interviews, offers, and managed-placement updates. Click an update to go straight to the next step.</p></div>{unread?<form action={markAllNotificationsReadAction}><button className="btn va-notifications-mark-all" type="submit">Mark all read</button></form>:null}</div>
    <div className="stack va-notifications-list">{notifications?.length?notifications.map((n:any)=><article className={`card notification-card va-notification-card ${n.read_at?"":"unread"}`} key={n.id}>
      <div className="row-between wrap va-notification-row" style={{gap:16}}>
