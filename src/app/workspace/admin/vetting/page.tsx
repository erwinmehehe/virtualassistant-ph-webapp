...
export default async function AdminVettingPage({ searchParams }: { searchParams: Promise<Record<string,string|undefined>> }){
  ...
  const pm=new Map(((profiles||[]) as {id:string;full_name:string|null}[]).map((x)=>[x.id,x])); const vm=new Map(((vas||[]) as VettingVaRow[]).map((x)=>[x.user_id,x])); const sm=new Map<string,ScorecardRow>(); for(const x of (scores||[]) as ScorecardRow[]) if(!sm.has(x.va_id)) sm.set(x.va_id,x);
