...
function age(value?:string|null){if(!value)return"";const h=Math.max(0,Math.floor((Date.now()-new Date(value).getTime())/3600000));return h<24?`${h}h in stage`:`${Math.floor(h/24)}d in stage`;}
...
export default async function RecruiterRolesPage(){
  ...
  const {data:jobs,error}=await admin.from("jobs").select("id,title,company_name,status,hiring_stage,hiring_stage_entered_at,target_start_date,recruiter_id,client_id,created_at").eq("recruiter_id",userId).order("updated_at",{ascending:false}).limit(250);
  if(error)throw error;
  const jobIds=(jobs||[]).map((j:any)=>j.id);
  const [{data:shortlist},{data:interviews},{data:offers},{data:rooms}]=await Promise.all([
    jobIds.length?admin.from("job_shortlist_candidates").select("job_id,shortlist_status,client_decision").in("job_id",jobIds):Promise.resolve({data:[] as any[]}),
    jobIds.length?admin.from("candidate_interviews").select("job_id,status").in("job_id",jobIds):Promise.resolve({data:[] as any[]}),
    jobIds.length?admin.from("placement_offers").select("job_id,status").in("job_id",jobIds):Promise.resolve({data:[] as any[]}),
    jobIds.length?admin.from("workrooms").select("id,job_id,placement_stage").in("job_id",jobIds):Promise.resolve({data:[] as any[]})
  ]);
  const open=(jobs||[]).filter((j:any)=>!["filled","closed"].includes(j.hiring_stage));
  const clientWaiting=open.filter((j:any)=>j.hiring_stage==="client_review").length;
  const interviewing=open.filter((j:any)=>j.hiring_stage==="interviewing").length;
  const recruiting=open.filter((j:any)=>["ready_to_recruit","sourcing","internal_review"].includes(j.hiring_stage)).length;
  ...
  return <>
    ...
    <section className="card" style={{marginTop:18}}><div className="row-between wrap"><div><h2 style={{margin:0}}>Hiring pipeline</h2><p className="small muted" style={{margin:"5px 0 0"}}>Old records no longer compete with live recruiting work. Filled and closed roles remain available as history.</p></div><BriefcaseBusiness size={20}/></div>
      {(jobs||[]).length?<div className="stack" style={{marginTop:14}}>{(jobs||[]).map((job:any)=>{const s=(shortlist||[]).filter((x:any)=>x.job_id===job.id);const i=(interviews||[]).filter((x:any)=>x.job_id===job.id);const o=(offers||[]).filter((x:any)=>x.job_id===job.id);const room=(rooms||[]).find((x:any)=>x.job_id===job.id);const sla=slaState(job.hiring_stage,job.hiring_stage_entered_at);return <Link href={`/workspace/recruiter/roles/${job.id}`} className="card" key={job.id}><div className="row-between wrap"><div><div className="row wrap"><span className="badge">{STAGES[job.hiring_stage]||job.hiring_stage}</span>{sla?<span className={`badge ${sla.late?"badge-danger":""}`}><Clock3 size={12}/>{sla.label}</span>:null}</div><h3 style={{margin:"8px 0 3px"}}>{job.title}</h3><p className="small muted" style={{margin:0}}>{job.company_name||"Client"} · {age(job.hiring_stage_entered_at)}</p></div><strong>Open control center →</strong></div><div className="row wrap" style={{marginTop:12}}><span className="small muted">{s.filter((x:any)=>x.shortlist_status==="proposed").length} internal</span><span className="small muted">{s.filter((x:any)=>x.shortlist_status==="released").length} client-visible</span><span className="small muted">{i.filter((x:any)=>x.status!=="cancelled").length} interviews</span><span className="small muted">{o.filter((x:any)=>!["declined","cancelled"].includes(x.status)).length} offers</span>{room?<span className="badge badge-success">Placement created</span>:null}</div></Link>})}</div>:<div className="empty">No roles are assigned to you.</div>}
    </section>
  </>;
}
