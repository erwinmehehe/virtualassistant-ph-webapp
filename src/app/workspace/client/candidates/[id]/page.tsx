...
export default async function CandidateReviewPage({params}:{params:Promise<{id:string}>}){
  ...
  if(job?.status!=="published")return <div className="client-candidate-review-page client-candidate-review-state"><div className="page-head client-candidate-review-head"><div><Link className="text-link small" href={`/workspace/client/candidates?role=${summary.job_id}`}>← Back to shortlist</Link><h1 style={{marginTop:8}}>Candidate profile is being prepared</h1><p>Your recruiter has selected this VA, but the role is not active for client review yet.</p></div></div></div>;
  if(!candidateAccessUnlocked(access?.access_status))return <div className="client-candidate-review-page client-candidate-access-page"><div className="page-head client-candidate-review-head"><div><Link className="text-link small" href={`/workspace/client/candidates?role=${summary.job_id}`}>← Back to shortlist</Link><h1 style={{marginTop:8}}>Candidate access required</h1><p>Private hiring evidence remains protected until candidate access is active.</p></div></div><CandidateAccessGate jobId={summary.job_id} access={access} applicantCount={0} releasedCount={releasedCount||0} returnTo={`/workspace/client/candidates/${id}`}/></div>;
