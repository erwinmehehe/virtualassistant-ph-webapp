...
export default async function ClientSupportPage({ searchParams }: { searchParams: Promise<Record<string,string|undefined>> }) {
  ...
  return <>
    ...
    {roomRows.length?<div className="stack" style={{marginTop:18}}>{roomRows.map((room)=>{const job:Partial<JobSummaryRow>=jobMap.get(room.job_id)||{};const va:Partial<AvatarProfileRow>=vaMap.get(room.va_id||"")||{};const recent=requests.filter((r)=>r.workroom_id===room.id);return <section className="card stack" key={room.id}>
      ...
      {recent.length?<div><h3>Recent requests</h3><div className="stack">{recent.slice(0,8).map((request)=><div className="review-answer" key={request.id}><div className="row-between wrap"><strong>{TYPE_LABELS[request.request_type]||String(request.request_type).replaceAll("_"," ")}</strong><span className={`badge ${badge(request.status)}`}>{STATUS_LABELS[request.status]||request.status}</span></div><p className="small" style={{margin:"7px 0"}}>{request.details}</p><div className="row wrap small muted"><span><CalendarClock size={12}/>{new Date(request.created_at).toLocaleDateString("en-PH")}</span>{request.resolution?<span>Client Success: {request.resolution}</span>:null}</div></div>)}</div></div>:null}
