...
export default async function AdminPaymentsPage({searchParams}:{searchParams:Promise<Record<string,string|undefined>>}){
  ...
  const [{data:payments},{data:workrooms},{data:timeEntries}]=await Promise.all([
  ...
  const timeByRoom=new Map<string,any[]>();
  for(const entry of timeEntries||[]){const current=timeByRoom.get(entry.workroom_id)||[];current.push(entry);timeByRoom.set(entry.workroom_id,current);}
  const ready=(workrooms||[]).map((room:any)=>{const entries=timeByRoom.get(room.id)||[];const hours=entries.reduce((sum:number,row:any)=>sum+Number(row.hours||0),0);const rate=Number(room.agreed_hourly_rate||room.jobs?.min_hourly_rate||0);return{room,entries,hours,rate,amount:hours*rate};}).filter((row:any)=>row.hours>0&&row.rate>0);
