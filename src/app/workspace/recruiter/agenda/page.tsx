...
function taskHref(task:any){if(task.subject_type==="job"&&task.subject_id)return `/workspace/recruiter/matching/${task.subject_id}`;if(task.subject_type==="va"&&task.subject_id)return `/workspace/recruiter/candidates/${task.subject_id}`;return task.href||null;}
...
export default async function RecruiterAgendaPage(){
  ...
  const ownedJobIds=(ownedJobs||[]).map((job:any)=>job.id);
  const jobMap=new Map((ownedJobs||[]).map((job:any)=>[job.id,job.title]));
  ...
  const clientInterviews=clientInterviewResult.data||[];

  const items:any[]=[
    ...(discoveries||[]).map((lead:any)=>({kind:"discovery",at:lead.discovery_scheduled_at,title:lead.company||lead.name||"Discovery call",bookedBy:lead.name||null,email:lead.email||null,subtitle:lead.service||"Client discovery",timezone:lead.timezone||"Asia/Manila",meetingUrl:lead.discovery_meeting_url,id:lead.id,duration:lead.discovery_duration_minutes||30})),
    ...(tasks||[]).map((task:any)=>({kind:"task",at:task.due_at,title:task.title,subtitle:task.description||"Recruiter task",priority:task.priority,href:taskHref(task),id:task.id})),
    ...(vettingInterviews||[]).map((row:any)=>({kind:"vetting_interview",at:row.recruiter_interview_at,title:row.candidate?.full_name||"VA candidate",subtitle:"Recruiter vetting interview",vaId:row.va_id,id:row.va_id})),
    ...clientInterviews.map((row:any)=>({kind:"client_interview",at:row.scheduled_at,title:row.candidate?.full_name||"VA candidate",subtitle:jobMap.get(row.job_id)||"Client candidate interview",timezone:row.timezone||"Asia/Manila",meetingUrl:row.meeting_url,jobId:row.job_id,id:row.id,duration:row.duration_minutes||30}))
      ...
      {days.map((day)=>{const dayItems=items.filter((item)=>ymdInManila(new Date(item.at))===day.key);return <section className="card dashboard-section-card" key={day.key}>
        ...
        {dayItems.length?<div className="compact-list">{dayItems.map((item:any)=><div className="row-between wrap" key={`${item.kind}-${item.id}-${item.at}`} style={{padding:"12px 0",borderBottom:"1px solid var(--border)"}}>
          <div>
            ...
            </>:item.kind==="client_interview"?<div className="small muted">Client/VA: {timeLabel(item.at,item.timezone)} ({item.timezone}) · Recruiter: {timeLabel(item.at,"Asia/Manila")} (Manila) · {item.duration} min</div>:<div className="small muted">{timeLabel(item.at,"Asia/Manila")} · Manila</div>}
          </div>
