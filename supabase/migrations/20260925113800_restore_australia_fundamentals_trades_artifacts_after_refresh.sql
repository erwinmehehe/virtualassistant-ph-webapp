...
targets as (
  select l.id,s.course_slug,s.template_title,s.template_text,s.checklist_items
...
rebuilt as (
  select
    ...
    || jsonb_build_array(
      jsonb_build_object(
        'type','exercise',
        'title','Build the lesson work product',
        'text',case
          when t.course_slug='australian-va-fundamentals'
            then 'Complete the client scenario as an Australian-business VA. Use verified facts, make dates and ownership explicit, and route specialist decisions instead of guessing.'
          else 'Complete the trade-business scenario as the admin or dispatch VA. Keep technical, safety, pricing, spend, remedy, GST, and accounting decisions with the authorised owner.'
        end,
        'deliverable','Complete the ' || t.template_title || ' using the template below and leave a decision-ready record another team member can continue.'
      ),
      jsonb_build_object('type','template','title',t.template_title,'text',t.template_text),
      jsonb_build_object('type','checklist','title','Before you submit','items',t.checklist_items)
    ) as content
  from targets t
  cross join lateral jsonb_array_elements(
    (select l2.content from public.training_lessons l2 where l2.id=t.id)
  ) with ordinality b(block,ord)
  group by t.id,t.course_slug,t.template_title,t.template_text,t.checklist_items
)
