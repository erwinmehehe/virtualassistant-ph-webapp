...
export default async function VaSupportPage({ searchParams }: { searchParams: Promise<Record<string,string|undefined>> }) {
  ...
  const {data:rooms,error}=await admin.from("workrooms").select("id,job_id,client_id,placement_stage,agreed_schedule,start_date").eq("va_id",user.id).neq("placement_stage","ended").order("created_at",{ascending:false});
  if(error)throw error;
  const roomIds=(rooms||[]).map((r:any)=>r.id);const jobIds=(rooms||[]).map((r:any)=>r.job_id);
  const [{data:jobs},{data:requests}]=await Promise.all([
    jobIds.length?admin.from("jobs").select("id,title,company_name,timezone").in("id",jobIds):Promise.resolve({data:[] as any[]}),
    roomIds.length?admin.from("placement_support_requests").select("*").in("workroom_id",roomIds).eq("requester_id",user.id).order("created_at",{ascending:false}).limit(100):Promise.resolve({data:[] as any[]})
  ]);
  ...
  return <>
    ...
    <div className="page-head"><div><div className="kicker">Active work</div><h1>Schedule & support</h1><p>Report leave, sickness, emergencies, delays, schedule changes, or placement concerns in one place.</p></div></div>
    <div className="info-banner"><LifeBuoy size={17}/><div><strong>Tell the agency early.</strong><p style={{margin:"4px 0 0"}}>Client Success can coordinate with the client, reduce surprises, and help before a temporary issue affects the placement.</p></div></div>

    {(rooms||[]).length?<div className="stack" style={{marginTop:18}}>{(rooms||[]).map((room:any)=>{const job:any=jobMap.get(room.job_id)||{};const recent=(requests||[]).filter((r:any)=>r.workroom_id===room.id);return <section className="card stack" key={room.id}>
