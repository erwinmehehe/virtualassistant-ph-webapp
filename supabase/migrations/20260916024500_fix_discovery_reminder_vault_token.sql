-- Vault's decrypted_secrets view exposes encrypted ciphertext in `secret` and
-- the usable plaintext value in `decrypted_secret`. Use only the plaintext
-- value when authenticating the Supabase scheduler request.
