...
export default async function ClientSuccessSupportPage({ searchParams }: { searchParams: Promise<Record<string,string|undefined>> }) {
  ...
  const {data:requests,error}=await admin.from("placement_support_requests").select("*").order("created_at",{ascending:false}).limit(300);
  if(error)throw error;
  const roomIds=[...new Set((requests||[]).map((r:any)=>r.workroom_id))];
  const {data:rooms}=roomIds.length?await admin.from("workrooms").select("id,job_id,client_id,va_id,client_success_owner_id,placement_stage,health_status,health_score").in("id",roomIds):{data:[] as any[]};
  const roomMap=new Map((rooms||[]).map((r:any)=>[r.id,r]));
  const jobIds=[...new Set((rooms||[]).map((r:any)=>r.job_id))];
  const profileIds=[...new Set((rooms||[]).flatMap((r:any)=>[r.client_id,r.va_id]).filter(Boolean))];
  const [{data:jobs},{data:people}]=await Promise.all([
    jobIds.length?admin.from("jobs").select("id,title,company_name,recruiter_id").in("id",jobIds):Promise.resolve({data:[] as any[]}),
    profileIds.length?admin.from("profiles").select("id,full_name").in("id",profileIds):Promise.resolve({data:[] as any[]})
  ]);
  const jobMap=new Map((jobs||[]).map((j:any)=>[j.id,j]));const peopleMap=new Map((people||[]).map((p:any)=>[p.id,p]));
  const visible=(requests||[]).filter((request:any)=>{const room:any=roomMap.get(request.workroom_id);const job:any=room?jobMap.get(room.job_id):null;return profile.role==="admin"||room?.client_success_owner_id===user.id||job?.recruiter_id===user.id;});
  const rank=(r:any)=>r.priority==="urgent"?0:r.priority==="high"?1:2;
  const active=visible.filter((r:any)=>["open","acknowledged"].includes(r.status)).sort((a:any,b:any)=>rank(a)-rank(b)||new Date(a.created_at).getTime()-new Date(b.created_at).getTime());
  const recentClosed=visible.filter((r:any)=>["resolved","declined"].includes(r.status)).slice(0,30);
  const requested=String(query.request||"");

  const render=(request:any,closed=false)=>{const room:any=roomMap.get(request.workroom_id)||{};const job:any=jobMap.get(room.job_id)||{};const requester=request.requester_role==="client"?peopleMap.get(room.client_id)?.full_name:peopleMap.get(room.va_id)?.full_name;return <article className={`card ${request.id===requested?"support-request-highlight":""}`} key={request.id}>
