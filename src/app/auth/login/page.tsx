...
export default async function LoginPage({ searchParams }: { searchParams: Promise<Record<string,string|undefined>> }) {
  ...
  return (
    <>
      {trainingLogin ? (
        <TrainingSiteHeader courseSlug={trainingCourseSlug} current="login"/>
      ) : (
        <SiteHeader/>
      )}

      <main id="main-content" className={trainingLogin ? "auth-page training-auth-page" : "auth-page"}>
        <div className="auth-card auth-card-wide">
          <h1>{trainingLogin ? "Welcome back to training" : "Welcome back"}</h1>
          <p className="muted">
            {trainingLogin
              ? "Log in to continue your free training and saved progress."
              : "Log in to continue to your VirtualAssistant.com.ph workspace."}
          </p>
          ...
          <form action={loginAction} className="stack">
            ...
            <div className="field">
              ...
              <input id="password" type="password" name="password" minLength={8} required autoComplete="current-password"/>
            </div>
            ...
            <TurnstileWidget/>
            <button className="btn btn-primary" type="submit" data-track="login_submit">
              ...
              <div className="auth-social-stack">
                ...
                {microsoftEnabled ? (
                  <form action={oauthAction}>
                    ...
                    <button className="btn auth-social-btn" type="submit">Microsoft</button>
                  </form>
                ) : null}
              </div>
            </>
          ...
          {showConfirmationRecovery ? (
            <div className="auth-confirmation-help">
              <div className="auth-confirmation-copy">
                <div>
                  <strong>Didn&apos;t get the confirmation email?</strong>
          ...
          <div className="auth-login-options">
            {trainingLogin ? (
              ...
              <>
                <Link className="btn" href={clientJoinHref}>Create client account</Link>
                <Link className="btn" href={vaJoinHref}>Join as a VA</Link>
              </>
            )}
          </div>
        </div>
