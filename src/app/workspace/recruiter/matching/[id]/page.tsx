...
export default async function RecruiterJobMatchingDetail({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<Record<string,string|undefined>>}){
  ...
  return <>
    {query.contact_sent?<div className="success-banner">Client follow-up email sent and logged.</div>:null}{query.role_archived?<div className="success-banner">Role closed and archived. History is retained.</div>:null}{query.contact_error?<div className="alert" role="alert">{query.contact_error}</div>:null}{query.recommendation_saved?<div className="success-banner">Client note saved.</div>:null}
