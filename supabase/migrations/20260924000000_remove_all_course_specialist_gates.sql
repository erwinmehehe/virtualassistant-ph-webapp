...
-- current editorial publication checks. Incomplete courses stay draft, but
    ...
    where assessment.course_id = course.id
      and (
        ...
        or (
          ...
          and (
            jsonb_typeof(assessment.rubric) <> 'array'
            or jsonb_array_length(assessment.rubric) < 4
            or jsonb_typeof(assessment.resource_pack) <> 'array'
            or jsonb_array_length(assessment.resource_pack) < 2
          )
        )
      )
  );
