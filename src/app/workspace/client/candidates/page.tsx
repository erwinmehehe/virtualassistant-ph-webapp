...
export default async function ClientCandidatesPage(){
  ...
  const next=offers?{title:`${offers} decision${offers===1?"":"s"} ready`,copy:"A candidate is waiting for your final hiring decision.",label:"Review decisions"}:interviews?{title:`${interviews} interview${interviews===1?"":"s"} in progress`,copy:"Review candidates and keep the interview process moving.",label:"Review interviews"}:readyToReview?{title:`${readyToReview} candidate${readyToReview===1?"":"s"} ready for review`,copy:"Compare role fit and decide who should move forward.",label:"Review candidates"}:{title:"We’re looking for candidates",copy:"Your recruiting team is matching approved Virtual Assistants to your role.",label:"View roles"};
  ...
  return <>
    ...
    <section className="card dashboard-section-card" id="browse-vas">
      <div className="dashboard-section-head"><div><h2>Browse approved Virtual Assistants</h2><p>Every profile here has passed vetting. Invite anyone to one of your published roles, and they decide whether to apply. Contact details stay private until candidate access is active.</p></div><Link className="btn btn-sm" href="/find-talent">Open full directory</Link></div>
