...
import { CalendarClock, CheckCircle2, HeartPulse, MessagesSquare, ShieldCheck } from "lucide-react";
...
export default async function ClientTeamPage({searchParams}:{searchParams:Promise<Record<string,string|undefined>>}){
  ...
  const {data:rooms,error}=await admin.from("workrooms").select("*").eq("client_id",user.id).order("created_at",{ascending:false});
  if(error)throw error;
  const active=(rooms||[]).filter((r:any)=>r.placement_stage!=="ended"&&r.status!=="completed");
  const jobIds=[...new Set(active.map((r:any)=>r.job_id))];
  const vaIds=[...new Set(active.map((r:any)=>r.va_id).filter(Boolean))];
  const csmIds=[...new Set(active.map((r:any)=>r.client_success_owner_id).filter(Boolean))];
  const roomIds=active.map((r:any)=>r.id);
  const [{data:jobs},{data:vas},{data:csms},{data:checkins}]=await Promise.all([
    jobIds.length?admin.from("jobs").select("id,title,hours_per_week,timezone").in("id",jobIds):Promise.resolve({data:[] as any[]}),
    vaIds.length?admin.from("profiles").select("id,full_name,avatar_url").in("id",vaIds):Promise.resolve({data:[] as any[]}),
    csmIds.length?admin.from("profiles").select("id,full_name").in("id",csmIds):Promise.resolve({data:[] as any[]}),
    roomIds.length?admin.from("placement_checkins").select("*").in("workroom_id",roomIds).order("due_at"):Promise.resolve({data:[] as any[]})
  ]);
  const jobMap=new Map((jobs||[]).map((x:any)=>[x.id,x]));
  const vaMap=new Map((vas||[]).map((x:any)=>[x.id,x]));
  const csmMap=new Map((csms||[]).map((x:any)=>[x.id,x]));
  ...
  return <>
    ...
    {active.length?<div className="stack">{active.map((room:any)=>{const job:any=jobMap.get(room.job_id)||{};const va:any=vaMap.get(room.va_id)||{};const csm:any=csmMap.get(room.client_success_owner_id)||{};const roomCheckins=(checkins||[]).filter((c:any)=>c.workroom_id===room.id);const pending=roomCheckins.filter((c:any)=>!c.client_signal&&c.status!=="skipped");const requested=roomCheckins.find((c:any)=>c.id===requestedCheckin&&!c.client_signal);const next=requested||pending[0];return <section className="card stack" key={room.id}>
      <div className="row-between wrap"><div className="row" style={{alignItems:"center"}}>{va.avatar_url?<img src={va.avatar_url} alt="" style={{width:48,height:48,borderRadius:"50%",objectFit:"cover"}}/>:null}<div><h2 style={{margin:"0 0 3px"}}>{va.full_name||"Virtual Assistant"}</h2><p className="small muted" style={{margin:0}}>{job.title||"Placement"}</p></div></div><div className="row wrap"><span className={`badge ${healthBadge(room.health_status)}`}><HeartPulse size={13}/>{HEALTH_LABELS[room.health_status]||room.health_status}{room.health_score!=null?` · ${room.health_score}`:""}</span><span className="badge">{STAGE_LABELS[room.placement_stage]||room.placement_stage}</span></div></div>
      ...
      <div className="row wrap"><Link className="btn btn-primary" href="/workspace/client/workroom"><MessagesSquare size={15}/> Open placement workroom</Link><Link className="btn" href="/workspace/client/messages">Message the team</Link></div>
