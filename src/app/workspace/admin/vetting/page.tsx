...
type VettingReviewRow = VettingStageRow & { recruiter_interview_at: string | null };
type ScorecardRow = { va_id: string; total_score: number | null; recommendation: string | null; created_at: string };
type VettingVaRow = { user_id: string; headline: string | null; primary_category: string | null; weekly_hours: number | null; hourly_rate: number | null };
...
export default async function AdminVettingPage({ searchParams }: { searchParams: Promise<Record<string,string|undefined>> }){
  ...
  const pm=new Map(((profiles||[]) as {id:string;full_name:string|null;avatar_url:string|null}[]).map((x)=>[x.id,x])); const vm=new Map(((vas||[]) as VettingVaRow[]).map((x)=>[x.user_id,x])); const sm=new Map<string,ScorecardRow>(); for(const x of (scores||[]) as ScorecardRow[]) if(!sm.has(x.va_id)) sm.set(x.va_id,x);
  const finalists=rows.filter((x)=>x.stage==="finalist"); const approved=rows.filter((x)=>["approved","bench"].includes(x.stage));
