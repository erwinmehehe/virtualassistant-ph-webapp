...
export default async function RecruiterCandidate({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<Record<string,string|undefined>>}){
  ...
  return <div className="recruiter-candidate-page">
    ...
    <div className="profile-layout recruiter-candidate-layout"><div className="stack recruiter-candidate-main">
      ...
      <TrainingCredentials credentials={trainingCredentials} heading="Verified training completed"/>
      ...
      <section className="card candidate-public-card">
        <div className="row-between">
          <div><div className="small muted">Public directory</div><h3>{publicState}</h3></div>
          <Globe2 size={20}/>
        </div>
        <p className="small muted">{publicLive?"This VA is currently visible to public visitors.":consentActive?"Consent is active. The checklist below shows what still blocks publication.":"Do not publish this VA until active public-profile consent is recorded."}</p>
        <div className="public-requirement-list">
          {visibilityRequirements.map((item)=><div className={item.done?"done":"missing"} key={item.label}><span>{item.done?"✓":"•"}</span><span>{item.label}</span></div>)}
        </div>
        <div className="candidate-consent-line">
          <strong>{consentActive?"Consent active":"Consent not active"}</strong>
          <span>{consentActive?"Version "+candidateVa.public_profile_consent_version+(consentDate?" · "+consentDate:""):"VA controls this from Profile → Public profile & privacy choice"}</span>
        </div>
        {publicListing?.slug?<Link className="btn btn-sm btn-primary" href={"/va/"+publicListing.slug} target="_blank">Open public profile</Link>:null}
      </section>
      <div className="card recruiter-contact-card"><div className="row"><Mail size={18}/><div><div className="small muted">VA account email</div>{vaEmail?<a className="contact-email-link" href={`mailto:${vaEmail}`}>{vaEmail}</a>:<strong>Not available</strong>}</div></div><p className="small muted">Recruiter-only contact detail.</p>{candidateVa.resume_path?<><a className="btn btn-sm" href={`/api/admin/va-resume/${id}`} target="_blank"><FileText size={15}/> Open private resume</a><form action={markVaReviewEvidenceAction}><input type="hidden" name="va_id" value={id}/><input type="hidden" name="kind" value="resume"/><input type="hidden" name="return_to" value={`/workspace/recruiter/candidates/${id}`}/><button className="btn btn-sm" type="submit">{vetting?.resume_reviewed_at?"Resume reviewed ✓":"Mark resume reviewed"}</button></form></>:<span className="small muted">No resume uploaded.</span>}</div>
