...
export default async function CompareCandidatesPage({searchParams}:{searchParams:Promise<Record<string,string|string[]|undefined>>}){
  ...
  let candidates:any[]=[];let excludedCount=0;
  if(ids.length){
    const {data:summaries}=await admin.from("applications").select("id,job_id,va_id,jobs!inner(id,title,status,client_id)").in("id",ids).eq("jobs.client_id",user.id);
    ...
    const accessMap=new Map((accessRows||[]).map((row:any)=>[row.job_id,row.access_status]));const releasedSet=new Set((releasedRows||[]).map((row:any)=>`${row.job_id}:${row.va_id}`));
    const allowedIds=(summaries||[]).filter((row:any)=>row.jobs?.status==="published"&&candidateAccessUnlocked(accessMap.get(row.job_id))&&releasedSet.has(`${row.job_id}:${row.va_id}`)).map((row:any)=>row.id);
    excludedCount=(summaries||[]).length-allowedIds.length;
    if(allowedIds.length){const {data}=await admin.from("applications").select("*,jobs!inner(id,title,status,client_id)").in("id",allowedIds).eq("jobs.client_id",user.id);candidates=data||[];}
  }
  ...
  return <>
    ...
    {candidates.length<2?<div className="card empty"><p>Select at least two recruiter-released candidates.</p><Link className="btn btn-primary" href="/workspace/client/candidates">Return to shortlist</Link></div>:<>
      ...
      <div className="compare-grid">{candidates.map((application:any)=>{const profile=application.profile_snapshot||{};const score=Number(application.match_score||0);return <article className="card stack" key={application.id}>
        <div><span className="badge">Recruiter released</span><h2 style={{margin:"10px 0 2px"}}>{profile.full_name||"Vetted Virtual Assistant"}</h2><p className="muted" style={{margin:0}}>{profile.headline||profile.primary_category||"Virtual Assistant"}</p></div>
