...
export default async function RecruiterTodayPage({searchParams}:{searchParams:Promise<Record<string,string|undefined>>}) {
  ...
  return <div className="dash-page">
    ...
    <section className="card dashboard-section-card">
      ...
      {queue.length ? <div className="dash-actions">
        {queue.map((item:any) => {
      ...
      </div> : <div className="dashboard-caught-up"><CheckCircle2 size={22}/><div><strong>You’re caught up.</strong><p>No urgent recruiter work is waiting right now.</p></div><Link className="btn btn-sm" href="/workspace/recruiter/leads">Open CRM</Link></div>}
    </section>
  </div>;
}
