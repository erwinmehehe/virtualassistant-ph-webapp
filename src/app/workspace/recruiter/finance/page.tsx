...
export default async function RecruiterFinancePage(){
  ...
  const {data:rooms}=jobIds.length?await admin.from("workrooms").select("id,job_id,placement_stage,status,client:profiles!workrooms_client_id_fkey(full_name),va:profiles!workrooms_va_id_fkey(full_name)").in("job_id",jobIds).neq("placement_stage","ended").order("created_at",{ascending:false}):{data:[] as any[]};
  const roomIds=(rooms||[]).map((room:any)=>room.id);
  const {data:profiles}=roomIds.length?await admin.from("placement_finance_profiles").select("workroom_id,expected_monthly_client_revenue,expected_monthly_va_compensation,payment_cost_percent,monthly_ops_cost,other_monthly_cost,exception_status,exception_reason,exception_review_note").in("workroom_id",roomIds):{data:[] as any[]};
  const profileMap=new Map((profiles||[]).map((profile:any)=>[profile.workroom_id,profile]));
