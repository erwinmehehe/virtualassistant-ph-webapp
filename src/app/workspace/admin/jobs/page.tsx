...
export default async function AdminJobsPage({searchParams}:{searchParams:Promise<Record<string,string|undefined>>}){
  ...
  const cm=new Map((commercials||[]).map((row:any)=>[row.job_id,row]));
  const recruiterMap=new Map((recruiters||[]).map((row:any)=>[row.id,row.full_name||"Recruiter"]));
  const rows=jobs||[];
  const exceptions=rows.filter((job:any)=>{
    const commercial=cm.get(job.id) as any;
  ...
  const standardPending=rows.filter((job:any)=>job.status==="pending"&&job.service_model!=="managed_service"&&!cm.has(job.id)).length;
  const unlinked=rows.filter((job:any)=>job.status==="pending"&&!job.client_id).length;
  ...
  return <>
    ...
    <div className="grid-3" style={{marginBottom:18}}>
      <div className="card"><div className="small muted">Admin exceptions</div><strong style={{fontSize:28}}>{exceptions.length}</strong><p className="small muted">Managed-service pricing requiring an admin decision</p></div>
    ...
    <div className="stack">{visible.length?visible.map((job:any)=>{const commercial=cm.get(job.id) as any;const owner=job.recruiter_id?recruiterMap.get(job.recruiter_id):null;return <article className="card" key={job.id}>
      <div className="row-between wrap"><div><div className="row wrap"><span className={`badge ${job.status==="published"?"badge-success":job.status==="pending"?"badge-warning":""}`}>{job.status==="published"?"Recruiting":job.status}</span><span className={`badge ${job.service_model==="managed_service"?"badge-warning":""}`}>{job.service_model==="managed_service"?"Managed service":"Curated placement"}</span>{commercial?<span className="badge">Terms: {String(commercial.commercial_status).replaceAll("_"," ")}</span>:null}<span className="small muted">{dateShort(job.created_at)}</span></div><h3 style={{margin:"8px 0 4px"}}>{job.title}</h3><div className="small muted">{job.company_name||"Client"} · {job.hours_per_week?`${job.hours_per_week} hrs/week`:"Hours not set"} · VA pay from {money(job.min_hourly_rate)}/hr · Owner: {owner||"Unassigned"}</div></div><Link className="btn btn-primary btn-sm" href={`/workspace/admin/jobs/${job.id}`}>{job.service_model==="managed_service"&&job.status==="pending"?"Resolve exception":"View role"}</Link></div>
