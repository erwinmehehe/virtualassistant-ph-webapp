...
export default async function ClientInterviewsPage({searchParams}:{searchParams:Promise<Record<string,string|undefined>>}){
  ...
  return <>
    ...
    <div className="stack">
      {active.length?active.map((row:any)=>{const job:any=jobMap.get(row.job_id)||{};const candidate=nameMap.get(row.va_id)||"VA candidate";const canFeedback=Boolean(row.scheduled_at&&new Date(row.scheduled_at).getTime()<=Date.now());return <section className="card" key={row.id}>
