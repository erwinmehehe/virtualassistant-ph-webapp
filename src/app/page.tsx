...
export default async function HomePage() {
  ...
  return <><SiteHeader/><main id="main-content">
    ...
    <section className="section section-alt"><div className="container">
      <div className="section-head"><h2>Browse virtual assistant services, industries, and software.</h2><p>Every page explains the tasks to delegate, the tools involved, and what to look for before you interview.</p></div>
      <div className="seo-link-grid">
        <div>
          <h3>By service</h3>
          <ul>{SERVICE_PAGES.slice(0, 12).map((page) => <li key={page.slug}><Link href={`/service/${page.slug}`}>{page.name}</Link></li>)}</ul>
          <Link className="text-link" href="/services">All {SERVICE_PAGES.length} services →</Link>
        </div>
        <div>
          <h3>By industry</h3>
          <ul>{INDUSTRIES.slice(0, 12).map((industry) => <li key={industry.slug}><Link href={`/industries/${industry.slug}`}>{industry.label}</Link></li>)}</ul>
          <Link className="text-link" href="/industries">All {INDUSTRIES.length} industries →</Link>
        </div>
        <div>
          <h3>By software</h3>
          <ul>{softwarePages.slice(0, 12).map((page) => <li key={page.slug}><Link href={`/software/${page.slug}`}>{page.name}</Link></li>)}</ul>
          <Link className="text-link" href="/software">All {softwarePages.length} software guides →</Link>
        </div>
      </div>
    </div></section>
