...
export default async function AdminJobsPage({ searchParams }: { searchParams: Promise<Record<string,string|undefined>> }){
  ...
  const eligibleForAutoQuote=(jobs||[]).filter((j:any)=>j.status==="pending"&&j.service_model!=="managed_service"&&j.client_id&&!cm.has(j.id)).length;
  return <><div className="page-head"><div><h1>Job review</h1><p>Open the full role evidence, confirm feasibility, then send the service fee for client acceptance before publication.</p></div></div>
    {params.auto_quoted?<div className="alert alert-success" style={{marginBottom:16}}>Quoted {params.auto_quoted} job{params.auto_quoted==="1"?"":"s"}. Nothing was published. Each client must approve the terms before recruiting begins.{Number(params.skipped_managed)?` ${params.skipped_managed} managed-service job(s) still need a manual margin decision.`:""}{Number(params.skipped_unlinked)?` ${params.skipped_unlinked} unlinked lead job(s) still need a client workspace.`:""}</div>:null}
    {params.auto_quote_error==="no_default_fee"?<div className="alert" style={{marginBottom:16}}>Set a default placement fee in Marketplace settings before automatic quoting.</div>:null}
    <div className="card" style={{marginBottom:18}}>
      <h3 style={{marginTop:0}}>Prepare straightforward quotes</h3>
      <p className="small muted" style={{marginBottom:14}}>Adds the standard curated-placement fee to eligible pending roles and asks the client to approve it. This never accepts terms or publishes a job on the client's behalf.</p>
      <form action={autoQuoteStraightforwardJobsFormAction}>
        <button className="btn btn-primary btn-sm" type="submit" disabled={!eligibleForAutoQuote}>Prepare {eligibleForAutoQuote} quote{eligibleForAutoQuote===1?"":"s"}</button>
      </form>
    </div>
