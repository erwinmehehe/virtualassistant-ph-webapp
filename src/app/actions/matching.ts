...
export async function saveJobShortlistAction(formData: FormData) {
  ...
  if (mode === "release") {
    ...
    if (stale.length) {
      return fail(`${stale.length} selected VA${stale.length === 1 ? " needs" : "s need"} a fresh availability confirmation before client release. Send an availability reminder, then try again after they confirm.`);
    }
  }
