  ...
  return <><SiteHeader/><main id="main-content" className="talent-directory-page">
    ...
    <section className="section directory-section"><div className="container">
      ...
      {pageVas.length ? <><div className="talent-directory-grid">{pageVas.map((va:any)=>{
        ...
        const requestHref = `/hire?talent=${encodeURIComponent(va.slug)}`;
        return <article className="talent-market-card" key={va.user_id}>
          ...
          <div className="cro-directory-actions"><Link className="btn talent-card-cta" href={`/va/${va.slug}`}>View profile <ArrowRight size={15}/></Link><Link className="btn btn-primary" href={requestHref}>Get candidates like this</Link></div>
        </article>;
