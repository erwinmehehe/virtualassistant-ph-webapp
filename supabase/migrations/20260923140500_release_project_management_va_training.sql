-- Release the reviewed Project Management for Virtual Assistants course.
...
where module_id in (
  ...
  where course_id = (select id from public.training_courses where slug = 'project-management-for-virtual-assistants')
);
...
set
  ...
  content_version = greatest(content_version, 2),
  updated_at = now()
where slug = 'project-management-for-virtual-assistants';
