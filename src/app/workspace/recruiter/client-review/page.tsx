...
export default async function RecruiterClientReviewPage() {
  ...
  const rows = released || [];
  const jobIds = [...new Set(rows.map((row: any) => row.job_id))];
  ...
  const activeJobs=(jobs||[]).filter((job:any)=>job.status!=="closed");
  const activityRows=activity||[];
  const groups=activeJobs.map((job:any)=>{
    const shortlist=rows.filter((row:any)=>row.job_id===job.id);
    const decisions=shortlist.filter((row:any)=>row.client_decision);
    const passed=shortlist.filter((row:any)=>row.client_decision==="pass").length;
    const interested=shortlist.filter((row:any)=>row.client_decision==="interested").length;
    const interviews=shortlist.filter((row:any)=>row.client_decision==="interview").length;
    const viewed=activityRows.find((row:any)=>row.subject_id===job.id&&row.action==="client_shortlist_viewed")?.created_at||null;
    const followup=activityRows.find((row:any)=>row.subject_id===job.id&&row.action==="client_shortlist_followup")?.created_at||null;
    const oldest=shortlist.map((row:any)=>row.released_at).filter(Boolean).sort()[0]||null;
  ...
  }).sort((a:any,b:any)=>Number(b.allPassed)-Number(a.allPassed)||b.hoursWaiting-a.hoursWaiting);

  const waiting=groups.filter((group:any)=>group.decisions.length<group.shortlist.length).length;
  const replacement=groups.filter((group:any)=>group.allPassed).length;
  return <>
    ...
    <div className="stack">{groups.map((group:any)=>{
