...
export default async function RecruiterCandidate({params}:{params:Promise<{id:string}>}){
  ...
  const [{data:profile},{data:va},{data:vetting},{data:attempt},{data:scorecards},{data:authUser}]=await Promise.all([
    admin.from("profiles").select("id,full_name,avatar_url,created_at,account_status").eq("id",id).eq("role","va").maybeSingle(),
  ...
  return <>
    <div className="page-head"><div><div className="row wrap"><span className="badge">{vettingStatusLabel(stage)}</span><span className="small muted">Profile {completion}% complete</span>{assignedToYou?<span className="badge badge-success">Assigned to you</span>:vetting?.recruiter_id?<span className="badge badge-warning">Assigned to another recruiter</span>:<span className="badge">Unassigned</span>}</div><div className="row" style={{marginTop:10,alignItems:"center"}}><PublicAvatar name={profile.full_name||"VA"} src={profile.avatar_url}/><div><h1 style={{margin:0}}>{profile.full_name||"VA candidate"}</h1><p style={{margin:"4px 0 0"}}>{va.headline||va.primary_category||"Virtual Assistant"}</p></div></div></div><div className="row wrap"><Link className="btn" href="/workspace/recruiter/talent">Back to VA directory</Link>{unassigned&&stage==="recruiter_review"?<form action={claimCandidateAction}><input type="hidden" name="va_id" value={id}/><button className="btn" type="submit">Claim candidate</button></form>:null}{vetting?.video_url?<a className="btn btn-primary" href={vetting.video_url} target="_blank" rel="noreferrer">Watch video intro</a>:null}</div></div>

    <div className="profile-layout"><div className="stack">
      <section className="card"><div className="row-between wrap"><div><h3 style={{margin:0}}>Structured profile</h3><p className="small muted" style={{margin:"5px 0 0"}}>Private recruiter view. This page is not exposed to clients or public visitors.</p></div><span className="badge"><ShieldCheck size={13}/> Internal</span></div><p>{va.bio||"No professional summary yet."}</p><div className="score-grid"><div><span>Category</span><strong>{va.primary_category||"Not set"}</strong></div><div><span>Experience</span><strong>{va.years_experience??0} years</strong></div><div><span>Availability</span><strong>{va.weekly_hours?`${va.weekly_hours} hrs/week`:"Not set"}</strong></div><div><span>Rate</span><strong>{va.hourly_rate?`USD ${va.hourly_rate}/hr`:"Not set"}</strong></div><div><span>Overlap</span><strong>{va.overlap_hours??0} hrs/day</strong></div></div><div style={{marginTop:16}}><div className="small muted">Skills</div><div className="pill-list" style={{marginTop:7}}>{uniqueStrings(va.skills).map((x,index)=><span className="badge" key={`${String(x)}-${index}`}>{x}</span>)}</div></div><div style={{marginTop:16}}><div className="small muted">Tools</div><div className="pill-list" style={{marginTop:7}}>{uniqueStrings(va.tools).map((x,index)=><span className="badge" key={`${String(x)}-${index}`}>{x}</span>)}</div></div></section>
