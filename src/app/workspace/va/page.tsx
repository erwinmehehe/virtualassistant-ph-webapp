...
export default async function VaDashboardPage({searchParams}:{searchParams:Promise<Record<string,string|undefined>>}){
  ...
  if(recruiterRequests.length){
    ...
    nextAction={title:"Get your profile ready for screening",copy:`Complete ${completion.next.label} to reach the ${VETTING_PROFILE_MIN}% profile threshold for recruiter vetting.`,href:completion.next.href,label:"Continue profile",icon:FileText};
  }else if(!vetted){
  ...
  return <div className="dash-page role-overview va-overview">
    ...
    <section className="card dashboard-section-card"><div className="dashboard-section-head"><div><h2>Recruiting pipeline</h2><p>Interest starts with recruiter review. Clients only see you after recruiter approval.</p></div><Link className="btn btn-sm" href="/workspace/va/applications">Open opportunities</Link></div><div className="pipeline-summary" aria-label="Recruiting pipeline">{[["Interest sent",pipeline.applied],["Recruiter shortlist",pipeline.shortlisted],["Interview",pipeline.interview],["Offer",pipeline.offered],["Placed",pipeline.hired]].map(([label,count])=><div className="pipeline-step" key={String(label)}><span>{label}</span><strong>{count}</strong></div>)}</div>{pipeline.rejected?<div className="small muted pipeline-footnote">{pipeline.rejected} opportunity{pipeline.rejected===1?" was":"ies were"} closed without placement.</div>:null}</section>
