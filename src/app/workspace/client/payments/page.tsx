...
export default async function ClientPaymentsPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  return <div className="client-payments-page">
    <div className="page-head client-payments-head"><div><h1>Payments</h1><p>Invoices your recruiter has sent for confirmed hours or placements. Pay securely by card, GCash, or Maya -- charged in PHP at the rate shown before you pay.</p></div></div>
    ...
    <div className="stack client-payments-list">
      {payments?.length ? payments.map((p: any) => <div className="card client-payment-card" key={p.id}>
        <div className="row-between wrap client-payment-summary">
          <div className="client-payment-copy">
          ...
          <div className="client-payment-amount" style={{ textAlign: "right" }}>
        ...
        {p.status === "paid" ? (
          <details className="client-payment-dispute" style={{ marginTop: 12 }}>
            <summary className="small muted" style={{ cursor: "pointer" }}>Report a problem with this payment</summary>
            <form action={fileDisputeAction} className="stack client-payment-dispute-form" style={{ marginTop: 10 }}>
