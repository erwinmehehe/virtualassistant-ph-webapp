...
type VaThreadRow = {
  ...
  client_id: string;
  applications: { jobs: { title: string; company_name?: string } | null } | null;
};
...
export default async function VaMessagesPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  const { data: threadData } = await supabase
    .from("conversations")
    .select("id,application_id,client_id,applications(jobs(title,company_name))")
    .eq("va_id", user.id)
    .order("created_at", { ascending: false });
  const rows = (threadData || []) as unknown as VaThreadRow[];
  ...
  return <>
    <MessageLiveSync role="va" userId={user.id} conversationIds={threadIds}/>
    ...
    {!rows.length ? <section className="workspace-empty-card" aria-labelledby="va-messages-empty-title">
      ...
      <div className="thread-list"><div className="thread-list-head"><strong>Conversations</strong><span className="thread-count">{rows.length}</span></div>{rows.map((thread)=><Link className={`thread-item ${thread.id === active ? "active" : ""}`} href={`/workspace/va/messages?thread=${thread.id}`} key={thread.id}><div className="row-between"><strong>{thread.applications?.jobs?.company_name || "Client"}</strong>{unread.get(thread.id) ? <span className="unread-dot" aria-label={`${unread.get(thread.id)} unread messages`}>{unread.get(thread.id)}</span> : null}</div><div className="small muted">{thread.applications?.jobs?.title || "Job conversation"}</div></Link>)}</div>
