...
export default async function ClientCompanyPage({searchParams}:{searchParams:Promise<Record<string,string|undefined>>}){
  ...
  return <div className="client-company-page">
    ...
    <div className="stack client-company-stack" style={{maxWidth:900}}>
      ...
      <form action={updatePublicCompanyVisibilityAction} className="card stack client-company-visibility">
        ...
        <label className="visibility-toggle client-company-visibility-toggle"><input type="checkbox" name="public_company_visible" defaultChecked={Boolean(company?.public_company_visible)}/><span><strong>Show my company publicly on job listings</strong><small>When enabled, public job pages may show your company name, logo, website, industry, location, description, and verified hiring signals. Private contact details are never included.</small></span></label>
        <div className="row-between wrap client-company-visibility-actions"><span className="small muted">Off means public roles use “Confidential Client”. Recruiters and your own workspace still keep the full company record.</span><button className="btn btn-primary" type="submit">Save public identity preference</button></div>
      </form>
    </div>
  </div>;
}
