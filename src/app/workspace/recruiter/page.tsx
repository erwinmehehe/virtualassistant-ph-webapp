...
export default async function RecruiterDashboard(){
  ...
  const [unreviewedRes,incompleteRes,readyRes,vettedHiddenRes,activeJobsRes,newAppsRes,unreadMessagesRes,openLeadsRes,jobsRes,releasedRes]=await Promise.all([
    ...
    admin.from("lead_intake").select("id,created_at").eq("status","new").order("created_at",{ascending:true}).limit(200),
  ...
  const openLeadIds=(openLeadsRes.data||[]).map((lead:any)=>lead.id);
  const {data:leadContactRows}=openLeadIds.length
    ? await admin.from("recruiter_activity").select("subject_id").eq("subject_type","lead").in("subject_id",openLeadIds).or("action.like.client_contact_%,action.eq.client_followup_sent")
    : {data:[] as any[]};
  const contactedLeadIds=new Set((leadContactRows||[]).map((row:any)=>row.subject_id));
  const uncontactedLeads=(openLeadsRes.data||[]).filter((lead:any)=>!contactedLeadIds.has(lead.id));
  const overdueLeadCount=uncontactedLeads.filter((lead:any)=>new Date(lead.created_at).getTime()<Date.now()-24*60*60*1000).length;
  ...
  const cards=[
    ...
    ["Open client leads",(openLeadsRes.data||[]).length,"/workspace/recruiter/leads",Mail,overdueLeadCount?`${overdueLeadCount} waiting more than 24 hours for first contact`:"Open enquiries and follow-ups"]
  ] as const;
  const today=[
    {priority:"urgent",title:"Client leads waiting for first contact",count:uncontactedLeads.length,copy:overdueLeadCount?`${overdueLeadCount} have been waiting more than 24 hours. Contact the oldest leads first.`:"Contact new hiring enquiries before they go cold.",href:"/workspace/recruiter/leads"},
